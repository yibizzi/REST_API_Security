        ...
        .then((patient) => {
          ...
          appointment
            ...
            .then(() =>
              res
                .status(201)
                .json({ message: "Apppointment created successfully" })
            )
...
const cancelAppointmentPatient = (patient, req, res) => {
  ...
  const indexReq = requests.indexOf(req.params.appointmentId);
  const indexApp = appointments.indexOf(req.params.appointmentId);
  var stateReq = false;
  var stateApp = false;
  if (indexReq !== -1) {
